...
async function run(options, done) {
    ...
    async function main(number, options) {
        options = await immer_1.produce(options, async (draft) => {
            draft.number =
                number ||
                    getPullRequestNumberFromBranch_(draft.currentBranch, options.config.pull_branch_name_prefix);
            draft.pullBranch = getBranchNameFromPullNumber_(number, options.config.pull_branch_name_prefix);
...
async function comment(options) {
    ...
    if (useEditor && utils_1.userLeftMsgEmpty(body)) {
...
async function list(options, user, repo) {
    ...
    if (pulls.length) {
        ...
        json.branches.forEach((branch, index, arr) => {
            ...
            const printTableView = options.config.pretty_print === undefined || Boolean(options.config.pretty_print);
            if (printTableView) {
